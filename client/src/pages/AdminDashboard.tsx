...
export default function AdminDashboard() {
  ...
  const areas = useMemo(() => Array.from(new Set([...bangladeshAreas.map((item) => item.en), ...inquiries.map((item) => item.project_location).filter((value): value is string => Boolean(value))])).sort(), [inquiries]);
  const filteredInquiries = useMemo(() => filterInquiries(inquiries, { query, status, service, area: area === "all" ? "" : area }), [inquiries, query, status, service, area]);
