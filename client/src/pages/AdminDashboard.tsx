...
import { Activity, ArrowLeft, Bell, CalendarDays, Download, FileText, Languages, LogOut, Mail, PanelLeftClose, PanelLeftOpen, Pencil, Plus, RefreshCw, Search, ShieldCheck, Trash2, UserRound } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [compactStatusLabels, setCompactStatusLabels] = useState(() => typeof window !== "undefined" && window.matchMedia("(max-width: 380px)").matches);
  ...
  useEffect(() => { const media = window.matchMedia("(max-width: 380px)"); const onChange = () => setCompactStatusLabels(media.matches); onChange(); media.addEventListener("change", onChange); return () => media.removeEventListener("change", onChange); }, []);
  ...
  const newRequests = inquiries.filter((item) => item.status === "new");
  ...
  function handleSwipeStart(event: TouchEvent) { setTouchStartX(event.touches[0]?.clientX ?? null); }
  function handleSwipeEnd(id: string, event: TouchEvent) { if (touchStartX === null) return; const delta = event.changedTouches[0]?.clientX - touchStartX; if (delta < -45) setSwipedId(id); if (delta > 45) setSwipedId(null); setTouchStartX(null); }
  ...
  return <main className={`admin-page admin-page--redesigned${sidebarCollapsed ? " admin-sidebar-collapsed" : ""}`}>
    ...
    <aside className="admin-toolbar admin-toolbar--redesigned"><div className="admin-page-links"><Link className="text-link" to="/"><ArrowLeft size={16} /> {isBn ? "ওয়েবসাইট" : "Website"}</Link><Link className="text-link" to="/admin"><Activity size={16} /> {isBn ? "ওভারভিউ" : "Overview"}</Link><Link className="text-link" to="/admin/inquiries"><FileText size={16} /> {isBn ? "সব ইনকোয়ারি" : "All inquiries"}</Link><Link className="text-link" to="/admin/blog">{isBn ? "ব্লগ" : "Blog"}</Link><Link className="text-link" to="/admin/applications">{isBn ? "আবেদন" : "Applications"}</Link></div><div className="admin-sidebar-actions"><button className="language-toggle" type="button" onClick={toggleLanguage}><Languages size={14} /> {isBn ? "English" : "বাংলা"}</button><button className="button button--small button--sand" onClick={() => void loadData()}><RefreshCw size={15} /> {isBn ? "রিফ্রেশ" : "Refresh"}</button><button className="button button--small button--brown" onClick={() => void signOut()}><LogOut size={15} /> {isBn ? "বের হন" : "Sign out"}</button></div><div className="admin-notification-wrap"><button className="admin-notification-trigger" type="button" onClick={() => setNotificationsOpen((current) => !current)} aria-expanded={notificationsOpen}><Bell size={15} /><span>{isBn ? "নতুন রিকোয়েস্ট" : "New requests"}</span>{newRequests.length > 0 && <strong>{newRequests.length}</strong>}</button>{notificationsOpen && <div className="admin-notification-panel"><div><strong>{isBn ? "নতুন অনুরোধ" : "New requests"}</strong><small>{newRequests.length} {isBn ? "টি অ্যাকশন প্রয়োজন" : "need attention"}</small></div>{newRequests.slice(0, 3).map((item) => <Link key={item.id} to="/admin/inquiries" onClick={() => setNotificationsOpen(false)}><span>{item.name}</span><small>{item.district || item.area_name || "New inquiry"}</small></Link>)}{newRequests.length === 0 && <p>{isBn ? "নতুন অনুরোধ নেই" : "No new requests"}</p>}<Link className="admin-notification-view-all" to="/admin/inquiries" onClick={() => setNotificationsOpen(false)}>{isBn ? "সব ইনকোয়ারি দেখুন" : "View all inquiries"}</Link></div>}</div><Link className="admin-sidebar-profile" to="/admin/account"><span className="admin-sidebar-avatar">{initials(profileName)}</span><span><strong>{profileName}</strong><small>{isBn ? "অ্যাডমিন অ্যাকাউন্ট" : "Admin account"}</small></span><Pencil size={14} /></Link></aside>
    ...
    <div className="admin-dashboard-grid">
      <section className="admin-section admin-section--inquiries"><div className="admin-section-heading admin-section-heading--split"><div><span className="section-label">{isBn ? "অনুরোধ / রিপোর্ট" : "INQUIRIES / REPORTING"}</span><h2>{isBn ? "ইনকোয়ারি ইনবক্স" : "Inquiry inbox"}</h2><p>{isBn ? "ফিল্টার করে রিপোর্ট ডাউনলোড করুন এবং প্রতিটি অবস্থার পরিবর্তন ট্র্যাক করুন।" : "Filter the inbox, download an offline report, and track every status change."}</p></div><button className="button button--turquoise" onClick={() => void exportFiltered()} disabled={loading}><Download size={16} /> {isBn ? "CSV ডাউনলোড" : "Export CSV"}</button></div><div className="inquiry-filters inquiry-filters--dashboard"><label className="search-field"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isBn ? "নাম, ফোন, এলাকা বা বার্তা খুঁজুন" : "Search name, phone, area, or message"} /></label><select value={status} onChange={(event) => setStatus(event.target.value as "all" | InquiryStatus)}><option value="all">{isBn ? "সব অবস্থা" : "All statuses"}</option><option value="new">{isBn ? "নতুন" : "New"}</option><option value="in_progress">{isBn ? "চলমান" : (compactStatusLabels ? "Active" : "In progress")}</option><option value="closed">{isBn ? "বন্ধ" : (compactStatusLabels ? "Done" : "Closed")}</option></select><select value={district} onChange={(event) => setDistrict(event.target.value)}><option value="all">{isBn ? "সব জেলা" : "All districts"}</option>{districts.map((item) => <option key={item} value={item}>{item}</option>)}</select><label className="date-filter"><CalendarDays size={15} /><input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} aria-label={isBn ? "শুরুর তারিখ" : "Start date"} /></label><label className="date-filter"><CalendarDays size={15} /><input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} aria-label={isBn ? "শেষের তারিখ" : "End date"} /></label></div>{loading ? <div className="admin-empty">{isBn ? "ইনকোয়ারি লোড হচ্ছে…" : "Loading inquiries…"}</div> : filteredInquiries.length === 0 ? <div className="admin-empty"><h3>{isBn ? "কোনো মিল পাওয়া যায়নি" : "No matching inquiries"}</h3><p>{isBn ? "সার্চ বা তারিখের পরিসর পরিবর্তন করে আবার চেষ্টা করুন।" : "Try changing your search or date range."}</p></div> : <div className="inquiry-list">{filteredInquiries.slice(0, 8).map((inquiry) => <article className={`inquiry-card${swipedId === inquiry.id ? " inquiry-card--swiped" : ""}`} key={inquiry.id} onTouchStart={handleSwipeStart} onTouchEnd={(event) => handleSwipeEnd(inquiry.id, event)}><div className="inquiry-card-top"><div><span className="inquiry-date">{new Date(inquiry.created_at).toLocaleString()}</span><h3>{inquiry.name}</h3></div><select className={`inquiry-status-select inquiry-status-${inquiry.status}`} value={inquiry.status} onChange={(event) => void updateStatus(inquiry.id, event.target.value as InquiryStatus)} aria-label={isBn ? "ইনকোয়ারির অবস্থা" : "Inquiry status"}><option value="new">{isBn ? "নতুন" : "New"}</option><option value="in_progress">{isBn ? "চলমান" : (compactStatusLabels ? "Active" : "In progress")}</option><option value="closed">{isBn ? "বন্ধ" : (compactStatusLabels ? "Done" : "Closed")}</option></select></div><div className="inquiry-contact"><span>{inquiry.district || "District not recorded"}</span><span>{inquiry.area_name || inquiry.project_location || "Area not recorded"}</span><a href={`tel:${inquiry.phone}`}>{inquiry.phone}</a></div><p>{inquiry.message}</p>{swipedId === inquiry.id && <div className="inquiry-swipe-actions"><button type="button" onClick={() => void updateStatus(inquiry.id, "in_progress")}>{isBn ? "চলমান" : "In progress"}</button><button type="button" onClick={() => void updateStatus(inquiry.id, "closed")}>{isBn ? "বন্ধ" : "Close"}</button></div>}</article>)}</div>}{filteredInquiries.length > 8 && <Link className="admin-more-link" to="/admin/inquiries">{isBn ? `আরও ${filteredInquiries.length - 8} টি দেখুন` : `View ${filteredInquiries.length - 8} more inquiries`} <ArrowLeft size={15} /></Link>}</section>
