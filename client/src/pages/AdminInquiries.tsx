...
import { ArrowLeft, CalendarDays, Download, Languages, LogOut, PanelLeftClose, PanelLeftOpen, RefreshCw, Search } from "lucide-react";
...
export default function AdminInquiries() {
  ...
  return <main className={`admin-page admin-page--redesigned admin-shared-page${sidebarCollapsed ? " admin-sidebar-collapsed" : ""}`}><header className="admin-header admin-header--redesigned"><button className="admin-sidebar-toggle admin-navbar-toggle" type="button" onClick={() => setSidebarCollapsed((current) => !current)} aria-label={sidebarCollapsed ? "Open sidebar" : "Hide sidebar"}>{sidebarCollapsed ? <PanelLeftOpen size={17} /> : <PanelLeftClose size={17} />}<span>{isBn ? "মেনু" : "Menu"}</span></button><div><span className="section-label">{isBn ? "অ্যাডমিন / ইনবক্স" : "ADMIN / INBOX"}</span><h1>{isBn ? "ইনকোয়ারি" : "Inquiries"}</h1></div></header><AdminSidebar isBn={isBn} toggleLanguage={toggleLanguage} collapsed={sidebarCollapsed} onToggle={() => setSidebarCollapsed((current) => !current)} onRefresh={() => void load()} onSignOut={() => void signOut()} active="inquiries" /> <div className="admin-toolbar"><Link className="text-link" to="/admin"><ArrowLeft size={16} /> {isBn ? "অ্যাডমিন হাবে ফিরুন" : "Back to admin hub"}</Link><span>{filtered.length}/{items.length} {isBn ? "টি অনুরোধ" : filtered.length === 1 ? "inquiry" : "inquiries"}</span></div><section className="admin-section"><div className="inquiry-filters"><label className="search-field"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isBn ? "নাম, ফোন, অবস্থান, ইমেইল বা বার্তা খুঁজুন" : "Search name, phone, location, email, or message"} /></label><select value={status} onChange={(event) => setStatus(event.target.value as "all" | InquiryStatus)}><option value="all">{isBn ? "সব অবস্থা" : "All statuses"}</option><option value="new">{isBn ? "নতুন" : "New"}</option><option value="in_progress">{isBn ? "চলমান" : (compactStatusLabels ? "Active" : "In progress")}</option><option value="closed">{isBn ? "বন্ধ" : (compactStatusLabels ? "Done" : "Closed")}</option></select><select value={service} onChange={(event) => setService(event.target.value)}><option value="all">{isBn ? "সব সেবা" : "All services"}</option>{services.map((item) => <option key={item}>{item}</option>)}</select><select value={district} onChange={(event) => setDistrict(event.target.value)}><option value="all">{isBn ? "সব জেলা" : "All districts"}</option>{districts.map((item) => <option key={item} value={item}>{item}</option>)}</select><label className="date-filter"><CalendarDays size={15} /><input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} aria-label={isBn ? "শুরুর তারিখ" : "Start date"} /></label><label className="date-filter"><CalendarDays size={15} /><input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} aria-label={isBn ? "শেষের তারিখ" : "End date"} /></label><button className="button button--turquoise button--small" type="button" onClick={() => void exportFiltered()}><Download size={15} /> {isBn ? "CSV" : "Export CSV"}</button></div>{loading ? <div className="admin-empty">{isBn ? "ইনকোয়ারি লোড হচ্ছে…" : "Loading inquiries…"}</div> : filtered.length === 0 ? <div className="admin-empty"><h2>{isBn ? "কোনো মিল পাওয়া যায়নি" : "No matching inquiries"}</h2><p>{isBn ? "অনুসন্ধান বা ফিল্টার পরিবর্তন করে আবার চেষ্টা করুন।" : "Try changing the search or filters."}</p></div> : <div className="inquiry-list">{filtered.map((inquiry) => <article className={`inquiry-card${swipedId === inquiry.id ? " inquiry-card--swiped" : ""}`} key={inquiry.id} onTouchStart={handleSwipeStart} onTouchEnd={(event) => handleSwipeEnd(inquiry.id, event)}><div className="inquiry-card-top"><div><span className="inquiry-date">{new Date(inquiry.created_at).toLocaleString()}</span><h2>{inquiry.name}</h2></div><select className={`inquiry-status-select inquiry-status-${inquiry.status}`} value={inquiry.status} onChange={(event) => void updateStatus(inquiry.id, event.target.value as InquiryStatus)} aria-label={isBn ? "অনুরোধের অবস্থা" : "Inquiry status"}><option value="new">{isBn ? "নতুন" : "New"}</option><option value="in_progress">{isBn ? "চলমান" : (compactStatusLabels ? "Active" : "In progress")}</option><option value="closed">{isBn ? "বন্ধ" : (compactStatusLabels ? "Done" : "Closed")}</option></select></div><div className="inquiry-contact"><a href={`tel:${inquiry.phone}`}>{inquiry.phone}</a>{(inquiry.district || inquiry.area_name || inquiry.project_location) && <span className="inquiry-location"><strong>{isBn ? "জেলা: " : "District: "}</strong>{inquiry.district || "Not recorded"}{inquiry.area_name && <><strong>{isBn ? " · এলাকা: " : " · Area: "}</strong>{inquiry.area_name}</>}{!inquiry.area_name && inquiry.project_location && <><strong>{isBn ? " · পুরোনো লোকেশন: " : " · Legacy location: "}</strong>{inquiry.project_location}</>}</span>}{inquiry.email && <a href={`mailto:${inquiry.email}`}>{inquiry.email}</a>}{inquiry.service && <span>{inquiry.service}</span>}</div><p>{inquiry.message}</p>{swipedId === inquiry.id && <div className="inquiry-swipe-actions"><button type="button" onClick={() => void updateStatus(inquiry.id, "in_progress")}>{isBn ? "চলমান" : "In progress"}</button><button type="button" onClick={() => void updateStatus(inquiry.id, "closed")}>{isBn ? "বন্ধ" : "Close"}</button></div>}</article>)}</div>}</section></main>;
}
