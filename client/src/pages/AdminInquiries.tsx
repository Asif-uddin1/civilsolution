...
export default function AdminInquiries() {
  ...
  const areas = useMemo(() => Array.from(new Set([...bangladeshAreas.map((item) => item.en), ...items.map((item) => item.project_location).filter((value): value is string => Boolean(value))])).sort(), [items]);
  const filtered = useMemo(() => filterInquiries(items, { query, status, service, area: area === "all" ? "" : area }), [items, query, status, service, area]);
