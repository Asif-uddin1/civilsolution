...
import { buildInquiryText, hasRequiredInquiryFields } from "@/lib/inquiry";
...
export default function Home() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!hasRequiredInquiryFields(form)) {
    ...
    const summary = buildInquiryText(form);
...
<nav className="desktop-nav" aria-label={isBn ? "প্রধান নেভিগেশন" : "Primary navigation"}>
            {navItems.map(([label, id]) => <button key={id} onClick={() => goTo(id)}>{label}</button>)}<Link className="nav-page-link" to="/completed-projects">{isBn ? "সম্পন্ন প্রকল্প" : "Completed projects"}</Link><Link className="nav-page-link" to="/careers">{isBn ? "ক্যারিয়ার" : "Careers"}</Link>
          </nav>
...
<section className="hero-section">
          ...
          <div className="container hero-layout">
            ...
            <div className="hero-visual">
              <div className="hero-image-wrap"><img src="/netlify-assets/civil-solution-hero.webp" alt={isBn ? "নির্মাণ সাইটে নকশা পর্যালোচনা করছেন প্রকৌশলী" : "Engineer reviewing plans at a construction site"} /><div className="hero-image-caption"><span>{isBn ? "মাঠ নোট / ০১" : "FIELD NOTE / 01"}</span><strong>{isBn ? "সাইটভিত্তিক কারিগরি সহায়তা" : "Site-first technical support"}</strong></div></div>
