...
import { bangladeshAreas, isBangladeshDistrict } from "@/content/areas";
import { buildInquiryText, buildInquiryProjectLocation, hasRequiredInquiryFields } from "@/lib/inquiry";
...
export default function Home() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", district: "", areaName: "", projectLocation: "", email: "", service: "", message: "" });
  ...
  const handleDistrictChange = (district: string) => {
    ...
    if (isBangladeshDistrict(district.trim())) window.requestAnimationFrame(() => areaNameRef.current?.focus());
  };
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const effectiveForm = { ...form, projectLocation: buildInquiryProjectLocation(form.district, form.areaName) };
    if (!isBangladeshDistrict(form.district.trim()) || !hasRequiredInquiryFields(effectiveForm)) {
    ...
    if (supabase) {
      const { error } = await supabase.from("inquiries").insert({
        ...
        area_name: effectiveForm.areaName.trim(),
